...
class ReadResource implements CRUD, RecordOperations
{
    ...
    protected function getEventProducts($invoice)
    {
        try {
            ...
            $products = ProductPlanment::with(['eventProduct' => function ($query) {
                ...
                $query->with(['measure:id,symbol', 'brand:id,name']);
                $query->select('products.id', 'products.name', 'products.consecutive', 'products.product_code', 'products.brand_id', 'products.measure_id');
            }, 'taxes:id,name,acronym,default_percent'])->where('planment_id', $planmentId);
    ...
    protected function getEmployees($invoice)
    {
        ...
        Log::info($invoice);
        try {
            $employees = Planment::with(['employees' =>  function ($query) {
                $query->with('thirds:id,names,surnames,business_name,identification,type_document');
            }])->where('invoice_id',$invoice)->get();

            return response()->json(['message' => 'read: ' . $invoice, 'data' => $employees], 200);
        } catch (QueryException $ex) {
            Log::error('Query error InvoiceResource@readResource:getInvoiceProduct: - Line:' . $ex->getLine() . ' - message: ' . $ex->getMessage());
            return response()->json(['message' => 'read q'], 500);
        } catch (\Exception $ex) {
