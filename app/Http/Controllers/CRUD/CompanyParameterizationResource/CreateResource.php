...
class CreateResource implements CRUD
{
    public function resource(Request $request)
    {
        ...
        try {
            ...
            DB::rollback();
            Log::error('Query error CompanyParameterization@createResource: ' . $ex->getMessage());
            return response()->json(['message' => 'create q'], 500);
        } catch (\Exception $ex) {
            // In case of error, roll back the transaction
            DB::rollback();
            Log::error('unknown error CompanyParameterization@createResource: ' . $ex->getMessage());
